...
void low(void *args){
	...
	// unlock resource
	rt_mutex_release(&mutexResource); // Increment
}
...
void med(void *args){
	...
	for (int i = 0; i<5 ; i++){
		busy_wait_us(TIME_UNIT);
		rt_printf("med busy waiting...\n");
	}
...
void high(void *args){
	...
	// Lock
	rt_mutex_acquire(&mutexResource, TM_INFINITE); // Decrement 
...
void barrier(void *args){
	...
	rt_mutex_create(&mutexResource, "Mutex Resource");
